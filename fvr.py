...
class FVR():
    def __init__(self, model_path='./snapshot.pth', threshold=0.75):
        ...
        self.roi_extract = Finger_crop(output_w=128, output_h=64, dataset='FTVein90DB')
...
if __name__ == '__main__':
    # img_path = '0004.gif_0.png'
    img_path = 'seed0117.png'
    img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
    img = cv2.rotate(img, cv2.ROTATE_90_COUNTERCLOCKWISE)
    roi_extract = Finger_crop(output_w=128, output_h=64, dataset='FVUSM')
    roi_img, edges = roi_extract.crop_finger(np.array(img))
    roi_img = cv2.resize(roi_img, (400, 200))
    roi_img = cv2.rotate(roi_img, cv2.ROTATE_90_COUNTERCLOCKWISE)
    cv2.imwrite('roi_1225.png', roi_img)
